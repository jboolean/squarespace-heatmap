...
var drawIndicator = function(pos, clicks, hovers, maxClicks, maxHovers) {
  ...
  el.style.left = pos.x - size/2;
  el.style.top = pos.y - size/2;
  ...
  // label.style.marginTop = size/2 - 8 + 'px';
  // label.style.marginLeft = size/2 - 10 + 'px';
  label.style.height = size + 'px';
  label.className = 'clicksonhover';
  // label.style.marginLeft = label.style.marginTop = size/2 + 'px';

  circle.onmouseenter = function() {
    label.className = 'clicksonhover active';
  };
  ...
  document.getElementById('indicators').appendChild(wrapper);
};
